...
class BM_Department(models.Model):
    _name = "bm.department"
    _description = "Departmentos de la empesa"
    _inherit = ['mail.thread']
    _order = "name"
    _rec_name = 'complete_name'

    name = fields.Char('Nombre del departmento', required=True)
    ...
    parent_id = fields.Many2one('bm.department', string='Departmento superior', index=True, domain="['|', ('company_id', '=', False), ('company_id', '=', company_id), ('parent_id', '!=', parent_id)]")
    child_ids = fields.One2many('bm.department', 'parent_id', string='Departmentos')
    manager_id = fields.Many2one('bm.official', string='Manager', tracking=True, domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]")
    member_ids = fields.One2many('bm.official', 'department_id', string='Miembros', readonly=True)
    jobs_ids = fields.One2many('bm.job', 'department_id', string='Puestos de trabajo')
    ...
    def _check_parent_id(self):
        if not self._check_recursion():
            raise ValidationError(_('No puede crear departamentos recursivos.'))
    ...
    def create(self, vals):
        ...
        department = super(BM_Department, self.with_context(mail_create_nosubscribe=True)).create(vals)
