...
_EstadosFuncionario = [('borrador', 'Borrador'), ('enproceso', 'En proceso de alta'), ('listo', 'Listo')]
...
class hr_employee_sudameris_inherit(models.Model):
  ...
  barrio = fields.Many2one(string='Barrio', comodel_name='res.country.neighborhood', domain="[('departament_id', '=?', departamento)]", required=True)
  departamento = fields.Many2one(string='Departamento', comodel_name='res.country.departament', domain="[('state_id', '=?', ciudad)]", required=True)
  ...
  _sql_constraints = [
      ...
      ('identification_id_uniq', 'unique (identification_id)', "The identification_id must be unique, this one is already assigned to another employee."),
  ...
  def _on_change_name(self):
    for funcionario in self:
      ...
      if (funcionario.nombre_2):
        _nombre = "{} {}".format(funcionario.nombre_1, funcionario.nombre_2)

      _apellido = funcionario.apellido_1
      if (funcionario.apellido_2):
        _apellido = "{} {}".format(funcionario.apellido_1, funcionario.apellido_2)
  ...
  def crear_movimientos(self):
    ...
    for funcionario in self.env['hr.employee'].browse(self._context.get('active_ids')):
      ...
      if funcionario.state == 'listo':
        list_movimientos = movimientos.search([('funcionario.id', '=', funcionario.id)])
        for movimiento in list_movimientos:
          if datetime.now().month == movimiento.fecha_pago.month:
            _errors.append('{} ya posee registro'.format(funcionario.name))
            _create = False
  ...
  def cliente_posee_cuenta(self):
    ...
    for funcionario in self.env['hr.employee'].browse(self._context.get('active_ids')):
      if funcionario.state == 'borrador':
        _changes.append('{} todavia no está aprobado, debe estar En Proceso de Alta'.format(funcionario.name))
        continue
      ...
      _changes.append('{} obtuvo su cuenta correctamente'.format(funcionario.name))
